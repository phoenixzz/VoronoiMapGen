...
const uint32 SPARKOpenGLES2QuadRenderer::SPARK_BUFFER_NAME = String("SPK_OpenGLES2QuadRenderer_Buffer").hashCode();


SPARKOpenGLES2QuadRenderer::SPARKOpenGLES2QuadRenderer(ISGPRenderDevice* device, float scaleX, float scaleY) :
...
bool SPARKOpenGLES2QuadRenderer::setTexturingMode(SPARK::TexturingMode mode)
{
	if( mode == SPARK::TEXTURE_NONE )
	{
		Logger::getCurrentLogger()->writeToLog(String("SPARKOpenGLES2QuadRenderer could not support TEXTURE_NONE mode when setting particle quad renderer"), ELL_ERROR);
...
void SPARKOpenGLES2QuadRenderer::createBuffers(const SPARK::Group& group)
{
	...
	if( (texturingMode == SPARK::TEXTURE_2D) && group.getModel()->isEnabled(SPARK::PARAM_TEXTURE_INDEX) )
	{
		if( group.getModel()->isEnabled(SPARK::PARAM_ANGLE) )
			renderParticle = &SPARKOpenGLES2QuadRenderer::renderAtlasRot;
		else
			renderParticle = &SPARKOpenGLES2QuadRenderer::renderAtlas;
	}
	else
	{
		if( group.getModel()->isEnabled(SPARK::PARAM_ANGLE) )
			renderParticle = &SPARKOpenGLES2QuadRenderer::renderRot;
		else
			renderParticle = &SPARKOpenGLES2QuadRenderer::renderBasic;
	}
...
void SPARKOpenGLES2QuadRenderer::render(const SPARK::Group& group)
{
	...
	SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* tmp_pVerts = (SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX*)DB->getVBO()->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
...
void SPARKOpenGLES2QuadRenderer::renderBasic(SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* pVertex, const SPARK::Particle& particle) const
{
	scaleQuadVectors(particle, scaleX*getInstanceScale(), scaleY*getInstanceScale());
	FillBufferColorAndVertex(pVertex, particle);
}

void SPARKOpenGLES2QuadRenderer::renderRot(SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* pVertex, const SPARK::Particle& particle) const
{
	rotateAndScaleQuadVectors(particle, scaleX*getInstanceScale(), scaleY*getInstanceScale());
	FillBufferColorAndVertex(pVertex, particle);
}

void SPARKOpenGLES2QuadRenderer::renderAtlas(SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* pVertex, const SPARK::Particle& particle) const
{
	scaleQuadVectors(particle, scaleX*getInstanceScale(), scaleY*getInstanceScale());
	FillBufferColorAndVertex(pVertex, particle);
...
void SPARKOpenGLES2QuadRenderer::renderAtlasRot(SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* pVertex, const SPARK::Particle& particle) const
{
	rotateAndScaleQuadVectors(particle, scaleX*getInstanceScale(), scaleY*getInstanceScale());
	FillBufferColorAndVertex(pVertex, particle);
	FillBufferTextureAtlas(pVertex, particle);
...
void SPARKOpenGLES2QuadRenderer::FillBufferColorAndVertex( SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* pVertex, const SPARK::Particle& particle ) const
{
...
void SPARKOpenGLES2QuadRenderer::FillBufferTextureAtlas(SGPVertex_UPOS_TEXTURE_VERTEXCOLOR_TEXINDEX* pVertex, const SPARK::Particle& particle) const
{

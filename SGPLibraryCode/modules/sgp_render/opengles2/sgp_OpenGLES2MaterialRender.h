...
#define __SGP_OPENGLES2MATERIALRENDER_HEADER__

class COpenGLES2RenderDevice;

// MaterialRenderer
class COpenGLES2MaterialRenderer
{
public :
	COpenGLES2MaterialRenderer(COpenGLES2RenderDevice *pRenderDevice);
	~COpenGLES2MaterialRenderer();


	// Init & Interface
	inline COpenGLES2RenderDevice*	GetOpenGLRenderDevice()	{ return m_pRenderDevice; };
	...
	void		DoDrawRenderBatch_SkinAnimAlphaTest();	// Render AlphaTest skeleton GPU Skin Anim render batch
	...
	void		DoDrawRenderBatch_AlphaTest();		// Render AlphaTest render batch ( include SkinAnim and normal alpha-test )
	...
	void		PushAlphaTestRenderBatch(const CAlphaTestRenderBatch& batch);
...
public:
	...
	AlphaTestRenderBatchVector			m_alphaTestBatch;
	AlphaTestRenderBatchVector			m_alphaTestLightMapBatch;
...
private:
	...
	OwnedArrayAlphaTestRenderBatch			m_AlphaTestRenderBatchPool;
	...
	static int m_AlphaTestRenderBatchPoolUsed;
...
// inlines
inline void COpenGLES2MaterialRenderer::PushMaterial( AbstractMaterial * material, SGP_MATERIAL_MERGE mode )
{
	materialStack_.add( MaterialInfo( material, mode ) );
}

inline void COpenGLES2MaterialRenderer::PopMaterial()
{
	materialStack_.removeLast();
}

inline void COpenGLES2MaterialRenderer::Draw( ISGPRenderBatch::BatchType BTtype, AbstractMaterial * material, SGP_MATERIAL_MERGE mode )
{
	materialStack_.add( MaterialInfo( material, mode ) );
	Draw( BTtype );
	materialStack_.removeLast();
...
#endif		// __SGP_OPENGLES2MATERIALRENDER_HEADER__
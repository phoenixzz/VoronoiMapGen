									...
									CPVRTMap<unsigned int, CPVRTMap<unsigned int, MetaDataBlock> > *pMetaData)
{
	...
	PVRTuint8 * ucpBuffer = 0;	
	PVRTuint32 iFileSize = 0;

	{	
		//Attempt to open file.
		ScopedPointer<FileInputStream> PVRTResource( File(String(filename)).createInputStream() );
		//Check file opened successfully.
		if( PVRTResource == nullptr )
		{
			return PVR_FAIL;
		}

		iFileSize = (PVRTuint32)PVRTResource->getTotalLength();

		ucpBuffer = new PVRTuint8 [iFileSize];

		PVRTResource->read(ucpBuffer, iFileSize);
	...
	EPVRTError Result = PVRTTextureLoadFromPointer(ucpBuffer, texName, psTextureHeader, bAllowDecompress, nLoadFromLevel,NULL,pMetaData);

...
uint32 COpenGLES2VertexCacheManager::StaticBuffer_ID = 0;
float COpenGLES2VertexCacheManager::m_fSin[CIRCLE_SLIDES_MAX] = {0};
float COpenGLES2VertexCacheManager::m_fCos[CIRCLE_SLIDES_MAX] = {0};

COpenGLES2VertexCacheManager::COpenGLES2VertexCacheManager(COpenGLES2RenderDevice* pRenderDevice) 
	: m_pRenderDevice(pRenderDevice), m_pFullScreenQuadVAO(NULL)
{
	...
	m_UPOSVCCache.add( new COpenGLES2DynamicBuffer(		INIT_DB_MAXSIZE, 
														INIT_DB_MAXSIZE,
														sizeof(SGPVertex_UPOS_VERTEXCOLOR),
														SGPVT_UPOS_VERTEXCOLOR,
														this ) );

	m_UPOSTEXCache.add( new COpenGLES2DynamicBuffer(	INIT_DB_MAXSIZE, 
														INIT_DB_MAXSIZE,
														sizeof(SGPVertex_UPOS_TEXTURE),
														SGPVT_UPOS_TEXTURE,
														this ) );

	m_UPOSTEXVCCache.add( new COpenGLES2DynamicBuffer(	INIT_DB_MAXSIZE, 
														INIT_DB_MAXSIZE,
	...
	*/
	...
	m_pFullScreenQuadVAO = new COpenGLES2VertexBufferObject(pRenderDevice);
	...
	m_pFullScreenQuadVAO->initVBOBuffer(SGPBT_VERTEX, quadVert, sizeof(SGPVertex_FONT) * 4, GL_STATIC_DRAW);
	...
	m_pFullScreenQuadVAO->initVBOBuffer(SGPBT_INDEX, quadIndex, sizeof(uint16) * 6, GL_STATIC_DRAW);

	m_pFullScreenQuadVAO->unBindVAO();
}

COpenGLES2VertexCacheManager::~COpenGLES2VertexCacheManager()
{
 ...
 */
void COpenGLES2VertexCacheManager::ForcedCommitAll(void)
{
	for( int i=0; i<m_UPOSVCCache.size(); i++ )
	{
 ...
 */
void COpenGLES2VertexCacheManager::ForcedClearAll(void)
{
	for( int i=0; i<m_UPOSVCCache.size(); i++ )
	{
		if( m_UPOSVCCache[i]->IsNotEmpty() )
 ...
 */
uint32 COpenGLES2VertexCacheManager::CreateStaticBuffer(
	...
	const uint16 *pIndis )
{
	...
	COpenGLES2StaticBuffer* pNewSB = new COpenGLES2StaticBuffer();
	...
	// create the VBO
	pNewSB->pVBO = new COpenGLES2VertexBufferObject( m_pRenderDevice );
	...
	pNewSB->pVBO->initVBOBuffer(SGPBT_VERTEX, pVerts, nVertexNum*pNewSB->nStride, GL_STATIC_DRAW);
	...
	pNewSB->pVBO->initVBOBuffer(SGPBT_INDEX, pIndis, nIndexNum*sizeof(uint16), GL_STATIC_DRAW);
 ...
 * -> OUT: uint32        - ID to the created StaticBuffer (0 for error)
 */
uint32 COpenGLES2VertexCacheManager::CreateMF1MeshStaticBuffer( 
	...
	uint32 NumBoneGroup )
{
		...
		pBoneGroup )
	{
		...
		pNewSB->nStride = sizeof(SGPVertex_ANIM);
		pNewSB->MaterialSkin.nShaderType = SGPST_SKELETONANIM;
	}
			 ...
			 (MF1Mesh.m_iNumUV0 > 0) && MF1Mesh.m_pTexCoords0 )
	{
		...
		if( (MF1Mesh.m_iNumVertexColor > 0) && MF1Mesh.m_pVertexColor )
		{
			...
			pNewSB->nStride = sizeof(SGPVertex_UPOS_TEXTURETWO_VC);
			pNewSB->MaterialSkin.nShaderType = SGPST_VERTEXCOLOR_LIGHTMAP;
		}
		else
		{
			VertexType = SGPVT_UPOS_TEXTURETWO;
			pNewSB->nStride = sizeof(SGPVertex_UPOS_TEXTURETWO);
			pNewSB->MaterialSkin.nShaderType = SGPST_LIGHTMAP;
		}
	}
	else if( MF1Mesh.m_iNumUV0 > 0 && MF1Mesh.m_pTexCoords0 )
	{
		...
		if( MF1Mesh.m_iNumVertexColor > 0 && MF1Mesh.m_pVertexColor )
		{
			...
			pNewSB->nStride = sizeof(SGPVertex_UPOS_TEXTURE_VERTEXCOLOR);
			pNewSB->MaterialSkin.nShaderType = SGPST_VERTEXCOLOR_TEXTURE;
		}
		else
		{
			VertexType = SGPVT_UPOS_TEXTURE;
			pNewSB->nStride = sizeof(SGPVertex_UPOS_TEXTURE);
			pNewSB->MaterialSkin.nShaderType = SGPST_TEXTURE;
		}
	}
	else
	{
	...
	case SGPVT_UPOS_VERTEXCOLOR:
		{
			...
			SGPVertex_UPOS_VERTEXCOLOR* tmp_pVerts = (SGPVertex_UPOS_VERTEXCOLOR *)pNewSB->pVBO->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
	...
	case SGPVT_UPOS_TEXTURE:
		{
			...
			// copy vertex data into the video memory
			SGPVertex_UPOS_TEXTURE* tmp_pVerts = (SGPVertex_UPOS_TEXTURE*)pNewSB->pVBO->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
			for( uint32 i=0; i<pNewSB->nNumVerts; i++ )
			{
				tmp_pVerts[i].x = MF1Mesh.m_pVertex[i].vPos[0];
				tmp_pVerts[i].y = MF1Mesh.m_pVertex[i].vPos[1];
	...
	case SGPVT_UPOS_TEXTURETWO:
		{
			...
			// copy vertex data into the video memory
			SGPVertex_UPOS_TEXTURETWO* tmp_pVerts = (SGPVertex_UPOS_TEXTURETWO*)pNewSB->pVBO->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
			for( uint32 i=0; i<pNewSB->nNumVerts; i++ )
			{
				tmp_pVerts[i].x = MF1Mesh.m_pVertex[i].vPos[0];
				tmp_pVerts[i].y = MF1Mesh.m_pVertex[i].vPos[1];
	...
	case SGPVT_UPOS_TEXTURETWO_VERTEXCOLOR:
		{
			...
			// copy vertex data into the video memory
			SGPVertex_UPOS_TEXTURETWO_VC* tmp_pVerts = (SGPVertex_UPOS_TEXTURETWO_VC*)pNewSB->pVBO->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
			for( uint32 i=0; i<pNewSB->nNumVerts; i++ )
			{
				tmp_pVerts[i].x = MF1Mesh.m_pVertex[i].vPos[0];
				tmp_pVerts[i].y = MF1Mesh.m_pVertex[i].vPos[1];
	...
	case SGPVT_UPOS_TEXTURE_VERTEXCOLOR:
		{
			...
			// copy vertex data into the video memory
			SGPVertex_UPOS_TEXTURE_VERTEXCOLOR* tmp_pVerts = (SGPVertex_UPOS_TEXTURE_VERTEXCOLOR*)pNewSB->pVBO->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
			for( uint32 i=0; i<pNewSB->nNumVerts; i++ )
			{
				tmp_pVerts[i].x = MF1Mesh.m_pVertex[i].vPos[0];
				tmp_pVerts[i].y = MF1Mesh.m_pVertex[i].vPos[1];
	...
	case SGPVT_ANIM:
		{
			...
			// copy vertex data into the video memory
			SGPVertex_ANIM* tmp_pVerts = (SGPVertex_ANIM*)pNewSB->pVBO->mapBufferToMemory(SGPBT_VERTEX, GL_WRITE_ONLY_OES);
			for( uint32 i=0; i<pNewSB->nNumVerts; i++ )
			{
				tmp_pVerts[i].x = MF1Mesh.m_pVertex[i].vPos[0];
				tmp_pVerts[i].y = MF1Mesh.m_pVertex[i].vPos[1];
	...
	pNewSB->pVBO->unmapBuffer(SGPBT_VERTEX);

	// copy index data into the video memory
	pNewSB->pVBO->bindVBO(SGPBT_INDEX);
	pNewSB->pVBO->initVBOBuffer(SGPBT_INDEX, MF1Mesh.m_pIndices, pNewSB->nNumIndis*sizeof(uint16), GL_STATIC_DRAW);
 ...
 **/
void COpenGLES2VertexCacheManager::RenderDynamicBuffer(
	SGP_VERTEX_TYPE VertexType,
	const SGPSkin& skin,
	uint32 nVertexNum, 
	uint32 nIndexNum, 
	...
	const uint16 *pIndis )
{
	Array<COpenGLES2DynamicBuffer*> *pCache = NULL;
	...
	COpenGLES2DynamicBuffer *pNewVC = NULL;
	// which vertex type is to be processed?
	switch(VertexType)
	{
    case SGPVT_UPOS_VERTEXCOLOR:		
		{ 
			pNewVC = new COpenGLES2DynamicBuffer(	INIT_DB_MAXSIZE, 
													INIT_DB_MAXSIZE,
													sizeof(SGPVertex_UPOS_VERTEXCOLOR),
													SGPVT_UPOS_VERTEXCOLOR,
													this );
...
void COpenGLES2VertexCacheManager::ClearAllStaticBuffer()
{
	for( int i=0; i<m_pSB.size(); i++ )
	{
		if( m_pSB[i] )
...
void COpenGLES2VertexCacheManager::ClearStaticBuffer( uint32 nSBufferID )
{
	if( nSBufferID == 0 )
		return;
...
void* COpenGLES2VertexCacheManager::GetStaticBufferByID(uint32 nSBufferID)
{
	int i = 0;
	for( i=0; i<m_pSB.size(); i++ )
	{
...
void COpenGLES2VertexCacheManager::RenderStaticBuffer( uint32 nSBufferID, const Matrix4x4& matWorld, const RenderBatchConfig& config )
{
	int i = 0;
	for( i=0; i<m_pSB.size(); i++ )
	{
	...
	COpenGLES2StaticBuffer *pStaticBuffer = m_pSB[i];
	...
	{
		//alpha-test
		CAlphaTestRenderBatch newRenderBatch(GetDevice());
		newRenderBatch.m_BBOXCenter = pStaticBuffer->BoundingBox.vcCenter;
		newRenderBatch.m_MatWorld = matWorld;
		newRenderBatch.m_BatchConfig = config;
		newRenderBatch.m_pSB = pStaticBuffer;
		newRenderBatch.m_pVC = NULL;
		newRenderBatch.BuildQueueValue();
		GetDevice()->getOpenGLMaterialRenderer()->PushAlphaTestRenderBatch(newRenderBatch);
	}
	else
	{
		//opaque
...
void COpenGLES2VertexCacheManager::RenderSkeletonMesh( uint32 nSBufferID, const Matrix4x4& matWorld, float* pBoneMatrixBuffer, uint32 nBoneNum, const RenderBatchConfig& config )
{
	int i = 0;
	for( i=0; i<m_pSB.size(); i++ )
	{
	...
	if( StaticBufferSkin.nShaderType == SGPST_SKELETONANIM )
	{
		...
		newRenderBatch.m_pSB = pStaticBuffer;
		newRenderBatch.m_pVC = NULL;
		newRenderBatch.m_BoneMatrixBuffer = pBoneMatrixBuffer;
		newRenderBatch.m_BoneNum = nBoneNum;
		newRenderBatch.BuildQueueValue();
...
void COpenGLES2VertexCacheManager::RenderPoints( uint32 nVertexNum, const SGPVertex_UPOS_VERTEXCOLOR *pVerts )
{
	SGPSkin pointSkin;
...
void COpenGLES2VertexCacheManager::RenderLines( uint32	nVertexNum,
...
void COpenGLES2VertexCacheManager::RenderLine(	const float  *fStart,
											const float  *fEnd, 
											const Colour *pColor )
{
	SGPSkin lineSkin;
...
void COpenGLES2VertexCacheManager::RenderTriangles( uint32 nVertexNum,
...
void COpenGLES2VertexCacheManager::FillTriangles( SGP_VERTEX_TYPE VertexType,
...
void COpenGLES2VertexCacheManager::RenderBox( const AABBox& aabbox, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderBox( const OBBox& obbox, const Colour& vColor )
{
	SGPVertex_UPOS_VERTEXCOLOR vb[8];
	uint16 ib[24] = { 0, 1, 1, 3, 3, 2, 2, 0, 4, 5, 5, 7, 7, 6, 6, 4, 0, 4, 1, 5, 2, 6, 3, 7 };
...
void COpenGLES2VertexCacheManager::RenderBox( const Vector3D& boxmin, const Vector3D& boxmax, const Colour& vColor )
{
	RenderBox( AABBox(boxmin, boxmax), vColor );
}

void COpenGLES2VertexCacheManager::FillBox( const AABBox& aabbox, const Colour& vColor )
{
	SGPVertex_UPOS_VERTEXCOLOR vb[8];
...
void COpenGLES2VertexCacheManager::FillBox( const OBBox& obbox, const Colour& vColor )
{
	SGPVertex_UPOS_VERTEXCOLOR vb[8];
	uint16 ib[36] = { 0, 1, 2, 1, 3, 2, 4, 6, 5, 5, 6, 7, 0, 5, 1, 0, 4, 5, 2, 3, 7, 2, 7, 6, 1, 7, 3, 1, 5, 7, 0, 2, 4, 2, 6, 4 };
...
void COpenGLES2VertexCacheManager::FillBox( const Vector3D& boxmin, const Vector3D& boxmax, const Colour& vColor )
{
	FillBox( AABBox(boxmin, boxmax), vColor );
}

void COpenGLES2VertexCacheManager::RenderCircle( const Vector3D& center, float radius, int axis, const Colour& vColor )
{
	RenderEllipse( center, Vector3D(radius, radius, radius), axis, vColor );
}

void COpenGLES2VertexCacheManager::RenderCircle( const Vector3D& center, float radius, const Vector3D& axis, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderEllipse( const Vector3D& center, const Vector3D& radius, int axis, const Colour& vColor )
{
	SGPVertex_UPOS_VERTEXCOLOR vb[CIRCLE_SLIDES_MAX+1];
	uint16 ib[CIRCLE_SLIDES_MAX*2];
...
void COpenGLES2VertexCacheManager::RenderEllipse( const Vector3D& center, const Vector3D& xaxis, float rx, const Vector3D& yaxis, float ry, const Colour& vColor )
{
	SGPVertex_UPOS_VERTEXCOLOR vb[CIRCLE_SLIDES_MAX+1];
	uint16 ib[CIRCLE_SLIDES_MAX*2];
...
void COpenGLES2VertexCacheManager::RenderDetailSphere( const Vector3D& center, float radius, int rings, int segments, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderSphere( const Vector3D& center, float radius, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderSphere( const Matrix4x4& matrix, float radius, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::FillSphere( const Vector3D& center, float radius, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderCylinder( const Vector3D& base, float radius, float height, int segments, const Colour& vColor )
{
	Vector3D start;
	Vector3D end;
...
void COpenGLES2VertexCacheManager::RenderCylinder( const Matrix4x4& matrix, float radius, float height, int segments, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderCone( const Vector3D& base, float radius, float height, int segments, const Colour& vColor )
{
	Vector3D start;
	Vector3D end;
...
void COpenGLES2VertexCacheManager::RenderCone( const Matrix4x4& matrix, float radius, float height, int segments, const Colour& vColor )
{
	Vector3D axis = Vector3D(matrix._21, matrix._22, matrix._23);
	Vector3D base = Vector3D(matrix._41, matrix._42, matrix._43);
	Vector3D top = base + axis * height;
...
void COpenGLES2VertexCacheManager::RenderEllipsoid( const Vector3D& center, const Vector3D& radius, const Colour& vColor )
{
	RenderEllipse(center, radius, 0, vColor);
	RenderEllipse(center, radius, 2, vColor);
}

void COpenGLES2VertexCacheManager::RenderEllipsoid( const Matrix4x4& matrix, const Vector3D& radius, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderCapsule( const Vector3D& p0, const Vector3D& p1, float radius, const Colour& vColor )
{
...
void COpenGLES2VertexCacheManager::RenderFrustum( const Frustum& f, const Colour& vColor )
{
	SGPVertex_UPOS_VERTEXCOLOR vb[8];
	uint16 ib[24] = { 0, 1, 1, 3, 3, 2, 2, 0, 4, 5, 5, 7, 7, 6, 6, 4, 0, 4, 1, 5, 2, 6, 3, 7 };
...
void COpenGLES2VertexCacheManager::RenderFullScreenQuad()
{
	if( m_pFullScreenQuadVAO )
	{
		...
		COpenGLES2ShaderManager *pShaderManager = static_cast<COpenGLES2ShaderManager*>(GetDevice()->GetShaderManager());
...
void COpenGLES2VertexCacheManager::RenderFullScreenQuadWithoutMaterial()
{
	if( m_pFullScreenQuadVAO )
	{		
		m_pFullScreenQuadVAO->bindVAO();

...
Matrix4x4 COpenGLCamera::m_mOpenGLProjTrans;
Matrix4x4 COpenGLCamera::m_mOpenGLProjTransInv;
 ...
 */
void COpenGLCamera::Prepare2DMode() 
{
...
void COpenGLCamera::SetClippingPlanes(float fNear, float fFar)
{
	m_fNear = fNear;
	m_fFar  = fFar;
 ...
 */
void COpenGLCamera::CalcPerspProjMatrix(float fAspect)
{
 ...
 */
void COpenGLCamera::GetFrustrum(Plane *p)
{
	// left plane
...
void COpenGLCamera::SetViewMatrix3D(
...
void COpenGLCamera::CalcOrthoProjMatrix(float l, float r, float b, float t, float fN, float fF)
{
	if(fabs(fF - fN) < 0.01f)
	{
		fN = fF;
...
void COpenGLCamera::CalcOrthoProjMatrix(float w, float h, float fN, float fF)
{
	if(fabs(fF - fN) < 0.01f)
	{
		fN = fF;

...
#define __SGP_OPENGLTERRAINRENDER_HEADER__

class COpenGLTerrainRenderer
{
public:
...
*/
	...
	COpenGLTerrainRenderer(COpenGLRenderDevice *pRenderDevice);
	~COpenGLTerrainRenderer();


	void setTerrainSize(uint32 terrainsize);
	void createChunkVBO(uint32 chunkindex);
	void flushChunkVBO(uint32 chunkindex);
	...
	void updateTerrainChunkLayerTexture(uint32 chunkindex, ESGPTerrainChunkTexture nLayer, const String& TextureName);
	// return updated world terrain Color Minimap Texture Data
	uint32* updateColorMinimapTexture(uint32* pChunkIndex, uint32 ichunkNum);

	void updateBlendTexture(uint32 SrcX, uint32 SrcZ, uint32 width, uint32 height, uint32* pData);
...
private:
	...
	GLuint							m_TerrainChunkAlphaBlendMapPBOID;
	GLuint							m_TerrainChunkColorMiniMapPBOID;
...
#endif		// __SGP_OPENGLTERRAINRENDER_HEADER__
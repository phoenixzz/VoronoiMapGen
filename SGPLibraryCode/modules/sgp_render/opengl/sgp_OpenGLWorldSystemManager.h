...
#define __SGP_OPENGLWORLDSYSTEMMANAGER_HEADER__

class COpenGLWorldSystemManager : public ISGPWorldSystemManager
{
public:
    COpenGLWorldSystemManager(COpenGLRenderDevice* pRenderDevice, Logger* pLogger);
	virtual ~COpenGLWorldSystemManager();

public:
	inline COpenGLRenderDevice*	GetDevice() 
	{ 
	...
	virtual ISGPObject* createObject( const String& MF1FileNameStr, const String& SceneObjectName,
		...
		float fScale = 1.0f, float fAlpha = 1.0f );
...
public:
	...
	virtual bool setHeightMap(uint32 index_x, uint32 index_z, uint16 iHeight);
	...
	virtual uint32* flushTerrainChunkColorMinimapTexture(uint32* pChunkIndex, uint32 ichunkNum);
	...
	virtual ISGPLightObject* createLightObject( const String& LightName, SGP_LIGHT_TYPE nLightType = SGPLT_Point,
		...
		const Colour& DiffuseColor = Colour(255, 255, 255) );
	...
	virtual bool flushSceneObject( ISGPObject* pObjArray, uint32 iObjNum, bool bRemove=false );
	...
	virtual uint32* updateTerrainLightmapTexture( float &fProgress, Random* pRandom );
	...
	//	\param pRandom						Specifies Random class	
	// NOTE: return pointer are alloced within this function, it should be free in other place
	virtual uint32* updateSceneObjectLightmapTexture( float &fProgress, ISGPObject* pSceneObj, uint32 nLMTexWidth, uint32 nLMTexHeight, Random* pRandom );
...
#endif		// __SGP_OPENGLWORLDSYSTEMMANAGER_HEADER__
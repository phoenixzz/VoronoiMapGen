...
private:
    ...
    SGP_DECLARE_NON_COPYABLE (WaitableEventImpl)
};
...
//==============================================================================
void SGP_CALLTYPE Thread::sleep (int millisecs)
{
    struct timespec time;
...
//==============================================================================
const sgp_wchar File::separator = '/';
...
namespace
{
   #if SGP_LINUX || (SGP_IOS && ! __DARWIN_ONLY_64_BIT_INO_T) // (this iOS stuff is to avoid a simulator bug)
    typedef struct stat64 sgp_statStruct;
    #define SGP_STAT     stat64
   #else
    typedef struct stat   sgp_statStruct;
    #define SGP_STAT     stat
   #endif

    bool sgp_stat (const String& fileName, sgp_statStruct& info)
    {
        return fileName.isNotEmpty()
                 && SGP_STAT (fileName.toUTF8(), &info) == 0;
    }

    // if this file doesn't exist, find a parent of it that does..
    bool sgp_doStatFS (File f, struct statfs& result)
    {
        for (int i = 5; --i >= 0;)
        {
            if (f.exists())
                                ...
                                Time* const modTime, Time* const creationTime, bool* const isReadOnly)
    {
        ...
        {
            sgp_statStruct info;
            const bool statOk = sgp_stat (path, info);
...
bool File::isDirectory() const
{
    ...
    return fullPath.isEmpty()
            || (sgp_stat (fullPath, info) && ((info.st_mode & S_IFDIR) != 0));
}
...
{
    sgp_statStruct info;
    return sgp_stat (fullPath, info) ? info.st_size : 0;
}
...
bool File::setFileTimesInternal (int64 modificationTime, int64 accessTime, int64 /*creationTime*/) const
{
    ...
    if ((modificationTime != 0 || accessTime != 0) && sgp_stat (fullPath, info))
    {
...
void FileOutputStream::flushInternal()
{
    if (fileHandle != 0)
    {
        ...
        android.activity.callVoidMethod (SGPAppActivity.scanFile, t.get());
       #endif
    }
}
...
File sgp_getExecutableFile()
{
    ...
    struct DLAddrReader
    {
        ...
        {
            Dl_info exeInfo;
            dladdr ((void*) sgp_getExecutableFile, &exeInfo);
...
//==============================================================================
void sgp_runSystemCommand (const String&);
void sgp_runSystemCommand (const String& command)
{
    int result = system (command.toUTF8());
    (void) result;
}

String sgp_getOutputFromCommand (const String&);
String sgp_getOutputFromCommand (const String& command)
{
    ...
    sgp_runSystemCommand (command + " > " + tempFile.getFullPathName());
...
public:
        ...
        : handle (0), refCount (1)
    {
        ...
        #if SGP_MAC
         File tempFolder ("~/Library/Caches/com.sgp.locks");
        #else
...
*/
//==============================================================================
void SGP_API sgp_threadEntryPoint (void*);
...
{
   #if SGP_IOS || (SGP_MAC && defined (MAC_OS_X_VERSION_10_5) && MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_5)
    SGP_AUTORELEASEPOOL
    [[NSThread currentThread] setName: SGPStringToNS (name)];
   #elif SGP_LINUX
...
private:
    ...
    SGP_DECLARE_NON_COPYABLE (ActiveProcess)
};

...
HWND sgp_messageWindowHandle = 0;  // (this is used by other parts of the codebase)

//==============================================================================
#if ! SGP_USE_INTRINSICS
// In newer compilers, the inline versions of these are used (in sgp_Atomic.h), but in
// older ones we have to actually call the ops as win32 functions..
long sgp_InterlockedExchange (volatile long* a, long b) noexcept                { return InterlockedExchange (a, b); }
long sgp_InterlockedIncrement (volatile long* a) noexcept                       { return InterlockedIncrement (a); }
long sgp_InterlockedDecrement (volatile long* a) noexcept                       { return InterlockedDecrement (a); }
long sgp_InterlockedExchangeAdd (volatile long* a, long b) noexcept             { return InterlockedExchangeAdd (a, b); }
long sgp_InterlockedCompareExchange (volatile long* a, long b, long c) noexcept { return InterlockedCompareExchange (a, b, c); }

__int64 sgp_InterlockedCompareExchange64 (volatile __int64* value, __int64 newValue, __int64 valueToCompare) noexcept
{
...
{
    if (sgp_messageWindowHandle != 0)
        AttachThreadInput (GetWindowThreadProcessId (sgp_messageWindowHandle, 0),
...
void SGP_CALLTYPE Thread::sleep (const int millisecs)
{
...
void sgp_repeatLastProcessPriority()
{
...
void Process::setPriority (ProcessPriority prior)
{
    if (lastProcessPriority != (int) prior)
    {
        ...
        sgp_repeatLastProcessPriority();
    }
}



bool SGP_CALLTYPE Process::isRunningUnderDebugger()
{

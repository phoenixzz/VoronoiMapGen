...
namespace WindowsFileHelpers
{
    ...
    File getModuleFileName (HINSTANCE moduleHandle)
    {
        ...
        dest[0] = 0;
        GetModuleFileName (moduleHandle, dest, (DWORD) (MAX_PATH + 256));
    ...
    Result getResultForLastError()
    {
        ...
        FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                       ...
                       messageBuffer, (DWORD) 255, nullptr);
...
int FileOutputStream::writeInternal (const void* pbuffer, int numBytes)
{
    if (fileHandle != nullptr)
    {
        DWORD actualNum = 0;
        if (! WriteFile ((HANDLE) fileHandle, pbuffer, (DWORD) numBytes, &actualNum, 0))
            status = WindowsFileHelpers::getResultForLastError();
...
//==============================================================================
File SGP_CALLTYPE File::getSpecialLocation (const SpecialLocationType type)
{
    int csidlType = 0;
...
File File::getCurrentWorkingDirectory()
{
    WCHAR dest [MAX_PATH + 256];
    dest[0] = 0;
    GetCurrentDirectory ((DWORD) (MAX_PATH + 256), dest);
    return File (String (dest));
}
...
void File::revealToUser() const
{
    ...
    SGP_LOAD_WINAPI_FUNCTION (dll, ILCreateFromPathW, ilCreateFromPathW, ITEMIDLIST*, (LPCWSTR))
    SGP_LOAD_WINAPI_FUNCTION (dll, ILFree, ilFree, void, (ITEMIDLIST*))
    SGP_LOAD_WINAPI_FUNCTION (dll, SHOpenFolderAndSelectItems, shOpenFolderAndSelectItems, HRESULT, (ITEMIDLIST*, UINT, void*, DWORD))

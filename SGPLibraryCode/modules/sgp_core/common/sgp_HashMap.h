...
#define __SGP_HASHMAP_HEADER__

#include "sgp_OwnedArray.h"
#include "sgp_LinkedListPointer.h"
#include "sgp_ScopedPointer.h"
...
public:
	...
	/** Generates a hash from a uint64. */
	static int generateHash (const uint64& key, const int upperLimit) noexcept	  { return std::abs ((int) key) % upperLimit; }
	/** Generates a hash from a memory address */
	static int generateHash (const void* key, const int upperLimit) noexcept	  { return std::abs ((intptr_t) key) % upperLimit; }
	/** Generates a simple hash from an 32 bits unsigned integer. */
	static int generateHash (const uint32 key, const int upperLimit) noexcept	  { return std::abs ((int) key) % upperLimit; }
...
private:
    ...
    public:
        ...
        SGP_DECLARE_NON_COPYABLE (HashEntry)
    };
...
public:
    ...
    */
    ...
    private:
        ...
        SGP_DECLARE_NON_COPYABLE (Iterator)
    };
...
private:
    ...
    SGP_DECLARE_NON_COPYABLE (HashMap)
};


#endif   // __SGP_HASHMAP_HEADER__

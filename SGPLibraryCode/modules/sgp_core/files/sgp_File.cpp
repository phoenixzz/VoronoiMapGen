...
File File::getChildFile (String relativePath) const
{
    ...
    if (relativePath[0] == '.')
    {
        ...
        {
            const sgp_wchar secondChar = relativePath[1];

            if (secondChar == '.')
            {
                const sgp_wchar thirdChar = relativePath[2];
...
String File::getRelativePathFrom (const File& dir)  const
{
...

    {
        ...
        for (int i = 0;;)
        {
            const sgp_wchar c1 = thisPathIter.getAndAdvance();
            const sgp_wchar c2 = dirPathIter.getAndAdvance();

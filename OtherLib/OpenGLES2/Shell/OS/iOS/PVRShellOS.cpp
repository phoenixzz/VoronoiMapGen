...
/*!***********************************************************************
 @Function		BeganTwoTouch
 @Description	Called from API when a two point touch is registered
*************************************************************************/

void PVRShellInitOS::BeganTwoTouch(float vec2Location[2][2], PVRShellInit* init)
{
	float Position[4] = { 	vec2Location[0][0] / (float)init->m_pShell->PVRShellGet(prefWidth) , vec2Location[0][1] / (float)init->m_pShell->PVRShellGet(prefHeight),
							vec2Location[1][0] / (float)init->m_pShell->PVRShellGet(prefWidth) , vec2Location[1][1] / (float)init->m_pShell->PVRShellGet(prefHeight) };
	init->TwoTouchBegan(Position);
}

/*!***********************************************************************
 @Function		MovedTwoTouch
 @Description	Called from API when a two point touch is registered
*************************************************************************/

void PVRShellInitOS::MovedTwoTouch(float vec2Location[2][2], PVRShellInit* init)
{
	float Position[4] = { 	vec2Location[0][0] / (float)init->m_pShell->PVRShellGet(prefWidth) , vec2Location[0][1] / (float)init->m_pShell->PVRShellGet(prefHeight),
							vec2Location[1][0] / (float)init->m_pShell->PVRShellGet(prefWidth) , vec2Location[1][1] / (float)init->m_pShell->PVRShellGet(prefHeight) };
	
	init->TwoTouchMoved(Position);
}

/*!***********************************************************************
 @Function		EndedTwoTouch
 @Description	Called from API when a two point touch is registered
*************************************************************************/

void PVRShellInitOS::EndedTwoTouch(float vec2Location[2][2], PVRShellInit* init)
{
	float Position[4] = { 	vec2Location[0][0] / (float)init->m_pShell->PVRShellGet(prefWidth) , vec2Location[0][1] / (float)init->m_pShell->PVRShellGet(prefHeight),
	 						vec2Location[1][0] / (float)init->m_pShell->PVRShellGet(prefWidth) , vec2Location[1][1] / (float)init->m_pShell->PVRShellGet(prefHeight) };
	
	init->TwoTouchEnded(Position);
}

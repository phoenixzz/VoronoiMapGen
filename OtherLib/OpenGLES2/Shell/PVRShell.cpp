...
void *PVRShell::PVRShellGet(const prefNamePtrEnum prefName) const
{
	...
	case prefPointerLocation:
		if (m_pShellInit->m_bTwoPointTouching)
			return m_pShellInit->m_vec2TwoPointerLocation;
...
/*!***********************************************************************
 @brief	     Used by the OS-specific code to tell the Shell that a two point touch has began at a location.
 @param[in]	 vec2Location   The two position of a click/touch on the screen when it first touches
*************************************************************************/
void PVRShellInit::TwoTouchBegan(const float vec2Location[4])
{
	m_bTwoPointTouching = true;
	m_vec2TwoPointerLocationStart[0] = m_vec2TwoPointerLocation[0] = vec2Location[0];
	m_vec2TwoPointerLocationStart[1] = m_vec2TwoPointerLocation[1] = vec2Location[1];
	m_vec2TwoPointerLocationStart[2] = m_vec2TwoPointerLocation[2] = vec2Location[2];
	m_vec2TwoPointerLocationStart[3] = m_vec2TwoPointerLocation[3] = vec2Location[3];

	KeyPressed(PVRShellKeyNameTWOTOUCH);
}

/*!***********************************************************************
 @brief	     Used by the OS-specific code to tell the Shell that a two point touch has began at a location.
 @param[in]	 vec2Location The two position of the pointer/touch pressed on the screen
*************************************************************************/
void PVRShellInit::TwoTouchMoved(const float vec2Location[4])
{
	if(m_bTwoPointTouching)
	{
		m_vec2TwoPointerLocation[0] = vec2Location[0];
		m_vec2TwoPointerLocation[1] = vec2Location[1];
		m_vec2TwoPointerLocation[2] = vec2Location[2];
		m_vec2TwoPointerLocation[3] = vec2Location[3];
...
/*!***********************************************************************
 @brief	    Used by the OS-specific code to tell the Shell that the current two point touch has ended at a location.
 @param[in] vec2Location The two position of the pointer/touch on the screen when it is released
*************************************************************************/
void PVRShellInit::TwoTouchEnded(const float vec2Location[4])
{
	if(m_bTwoPointTouching)
	{
		m_bTwoPointTouching = false;
		m_vec2TwoPointerLocationEnd[0] = m_vec2TwoPointerLocation[0] = vec2Location[0];
		m_vec2TwoPointerLocationEnd[1] = m_vec2TwoPointerLocation[1] = vec2Location[1];
		m_vec2TwoPointerLocationEnd[2] = m_vec2TwoPointerLocation[2] = vec2Location[2];
		m_vec2TwoPointerLocationEnd[3] = m_vec2TwoPointerLocation[3] = vec2Location[3];
	}
}

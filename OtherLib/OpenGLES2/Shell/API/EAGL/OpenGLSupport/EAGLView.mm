...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	{
		float Position[2][2] = {0};
		float* pPos = &(Position[0][0]);
		for( UITouch *touch in allTouches )
		{
			...
			*(pPos++) = touchLocation.x;
			*(pPos++) = touchLocation.y;
		}
		m_pPVRShellInit->BeganTwoTouch(Position, m_pPVRShellInit);
		return;
	}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	if( allTouches.count == 2 )
	{
		...
		m_pPVRShellInit->MovedTwoTouch(Position, m_pPVRShellInit);
		return;
	}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	if( allTouches.count == 2 )
	{
		...
		m_pPVRShellInit->EndedTwoTouch(Position, m_pPVRShellInit);
		return;
	}
